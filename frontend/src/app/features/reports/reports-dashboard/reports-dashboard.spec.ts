...
describe('ReportsDashboard', () => {
  ...
  const mockDashboardSummary: DashboardSummary = {
    ...
    recentStats: [{ date: '2024-01-01', orderCount: 10, revenue: 500 }],
  };
  ...
  describe('loadDashboardData', () => {
    ...
    it('should handle dashboard summary error', () => {
      ...
      expect(component.loading).toBe(false);
      expect(console.error).toHaveBeenCalledWith('Error loading dashboard:', errorResponse);
    });

    it('should handle latest report error gracefully', () => {
      mockReportService.getDashboardSummary.and.returnValue(of(mockDashboardSummary));
      mockReportService.getLatestReport.and.returnValue(throwError(() => ({ status: 404 })));
      mockReportService.getDailyStats.and.returnValue(of(mockDailyStats));
    ...
    it('should handle daily stats error gracefully', () => {
      mockReportService.getDashboardSummary.and.returnValue(of(mockDashboardSummary));
      mockReportService.getLatestReport.and.returnValue(of(mockLatestReport));
      mockReportService.getDailyStats.and.returnValue(throwError(() => ({ status: 500 })));
  ...
  describe('getMaxRevenue', () => {
    ...
    it('should handle single stat entry', () => {
      component.dailyStats = [{ date: '2024-01-01', orderCount: 10, revenue: 500 }];
  ...
  describe('DOM rendering', () => {
    ...
    it('should display error message', () => {
      ...
      expect(errorElement?.textContent).toContain('Failed to load dashboard data');
    });
