...
object OrderMain {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val rootBehavior = Behaviors.setup[Nothing] { context =>
      ...
      val serviceClient = new ServiceClient(productServiceUrl, customerServiceUrl, paymentServiceUrl)
      val streamProcessor = new OrderStreamProcessor(orderRepository, serviceClient)
      val orderActor = context.spawn(OrderActor(orderRepository, serviceClient, streamProcessor), "order-actor")
